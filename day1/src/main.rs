...
fn main() {
    let mut input = "76xkqjzqtwonfour
...
";
    //input = "4txvpps9kvonejhgsqpcv6eight2jxk\nsevensixjczjhjzbj8fnsnrsevenfive2seven";
    let vector = input
        .lines()
        .map(|x| format!("{}\n", x))
        .collect::<Vec<String>>();
    ...
    for i in vector {
        ...
        let mut digit_at_smallest_index = 0;
        for (index, digit) in [
        ...
        .enumerate()
        {
            let find = i.find(&digit.to_string());
            if find.is_some() {
                if find.unwrap() < smallest_index {
                    smallest_index = find.unwrap() as usize;
                    digit_at_smallest_index = index + 1;
                }
            }
        }
        ...
        for (index, c) in i.chars().enumerate() {
            if (index > smallest_index) {
                first = char::from_digit(digit_at_smallest_index as u32, 10).unwrap();
        ...
        //pirmais no aizmugures
        let mut largest_index: usize = 0;
        let mut digit_at_largest_index = 0;
        for (index, digit) in [
            "one", "two", "three", "four", "five", "six", "seven", "eight", "nine",
        ]
        .iter()
        .enumerate()
        {
            let find = i.rfind(&digit.to_string());
            if find.is_some() {
                println!("{} digit {}", find.unwrap(), digit);
                if find.unwrap() > largest_index {
                    largest_index = find.unwrap() as usize;
                    digit_at_largest_index = index + 1;
                }
            }
        }
        println!("largest index {} {}", largest_index, digit_at_largest_index);
        for (index, c) in i.chars().rev().enumerate() {
            if (i.len() - 1 - index < largest_index) {
                last = char::from_digit(digit_at_largest_index as u32, 10).unwrap();
        ...
        println!("pirmais {} pedejais {}", first, last);
        let a = first.to_string() + &last.to_string();
        println!("{}", a);
